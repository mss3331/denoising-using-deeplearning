"""
The following implementation is taken from github: https://github.com/say4n/pytorch-segnet/blob/f7738c6bce384b54fcbb3fe8aff02736d6ec2285/src/model.py#L333
...
class SegNetGRU(nn.Module):
    '''after each stage do the following
    1-save the orignal shape of the stage features
    2-create two views "row_view,column" of size (shape[0],-1,shape[1]) #batch,sequence(H*W),channels). row view needs to be transposed and used with reshape
    3-convert the GRU output to match the original shape
    4-concat output from row column and original stage features
    '''
    def __init__(self, input_channels, num_classes):
        super(SegNetGRU, self).__init__()
        ...
        # self.rnn_2_column = nn.GRU(input_size=128, hidden_size=64, num_layers=1, batch_first=True)

        self.encoder_conv_11 = nn.Sequential(*[
                                                nn.Conv2d(in_channels=128+64, # add GRU feature map
                                                          out_channels=128,
                                                          kernel_size=3,
                                                          padding=1),
                                                nn.BatchNorm2d(128)
        ...
        self.encoder_conv_22 = nn.Sequential(*[
                                                nn.Conv2d(in_channels=256+128, #add GRU feature map
                                                          out_channels=256,
                                                          kernel_size=3,
                                                          padding=1),
                                                nn.BatchNorm2d(256)
        ...
        # ****************** MAX POOL ****************************#
        # self.rnn_3_row = nn.GRU(input_size=512, hidden_size=256, num_layers=1, batch_first=True)
        # self.rnn_3_column = nn.GRU(input_size=512, hidden_size=256, num_layers=1, batch_first=True)
        # finish stage4
        self.encoder_conv_40 = nn.Sequential(*[
                                                nn.Conv2d(in_channels=512,
                                                          out_channels=512,
        ...
        # de GRU stage 5
        self.de_gru_5_row = nn.GRU(input_size=512, hidden_size=256, num_layers=1, batch_first=True)
        self.decoder_convtr_40 = nn.Sequential(*[
        ...
        # de GRU stage 4
        self.de_gru_4_row = nn.GRU(input_size=512, hidden_size=256, num_layers=1, batch_first=True)
        self.decoder_convtr_30 = nn.Sequential(*[
                                                nn.ConvTranspose2d(in_channels=512+256,
                                                                   out_channels=256,
                                                                   kernel_size=3,
        ...
        # de GRU stage 3
        self.de_gru_3_row = nn.GRU(input_size=256, hidden_size=128, num_layers=1, batch_first=True)
        self.decoder_convtr_20 = nn.Sequential(*[
        ...
        # de GRU stage 2
        self.de_gru_2_row = nn.GRU(input_size=128, hidden_size=64, num_layers=1, batch_first=True)
        self.decoder_convtr_10 = nn.Sequential(*[
    ...
    def forward(self, input_img):
        ...
        x_10 = F.relu(self.encoder_conv_10(x_0))
        # GRU at stage 2
        x_10 = self.gruUnit(x_10, self.gru_2_row)
        ...
        x_21 = F.relu(self.encoder_conv_21(x_20))
        # GRU at stage 3
        x_21 = self.gruUnit(x_21, self.gru_3_row)
        ...
        x_31 = F.relu(self.encoder_conv_31(x_30))
        # GRU at stage 4
        x_31 = self.gruUnit(x_31, self.gru_4_row)
        ...
        x_41 = F.relu(self.encoder_conv_41(x_40))
        # GRU at stage 5
        x_41 = self.gruUnit(x_41, self.gru_5_row)
        ...
        x_41d = F.relu(self.decoder_convtr_41(x_42d))
        # de GRU at stage 5
        x_41d = self.gruUnit(x_41d, self.de_gru_5_row)
        ...
        x_31d = F.relu(self.decoder_convtr_31(x_32d))
        # de GRU at stage 4
        x_31d = self.gruUnit(x_31d, self.de_gru_4_row)
        ...
        x_21d = F.relu(self.decoder_convtr_21(x_22d))
        # de GRU at stage 3
        x_21d = self.gruUnit(x_21d, self.de_gru_3_row)
        ...
        x_11d = F.relu(self.decoder_convtr_11(x_1d))
        # de GRU at stage 2
        x_11d = self.gruUnit(x_11d, self.de_gru_2_row)
    ...
    def gruUnit(selfe,x,gru_model):
        stage2_shape = x.shape  # store the original feature shape
        x_gru = x.view(stage2_shape[0], -1,stage2_shape[1])  # (batch_size, timesteps, channels"feature vectore for each Xt")
        x_gru, h_11 = gru_model(x_gru)  # get results after feeding it viewed feature map
        x_gru = x_gru.reshape(stage2_shape[0], -1, stage2_shape[2], stage2_shape[3])  # convert GRU result shape
        x_10 = torch.cat((x, x_gru), 1)
        return x_10

class SegNetGRU_Symmetric(nn.Module):
    '''after each stage do the following
    1-save the orignal shape of the stage features
    2-create two views "row_view,column" of size (shape[0],-1,shape[1]) #batch,sequence(H*W),channels). row view needs to be transposed and used with reshape
    3-convert the GRU output to match the original shape
    ...
    def __init__(self, input_channels, num_classes):
        super(SegNetGRU_Symmetric, self).__init__()
    ...
    def forward(self, input_img):
        ...
        x_42d = F.relu(self.decoder_convtr_42(x_4d))
        # de GRU at stage 5
        x_42d = self.gruUnit(x_42d, self.de_gru_5_row)
        # Finish de GRU at stage 5
        x_41d = F.relu(self.decoder_convtr_41(x_42d))
        x_40d = F.relu(self.decoder_convtr_40(x_41d))
        dim_4d = x_40d.size()
        ...
        x_32d = F.relu(self.decoder_convtr_32(x_3d))
        # de GRU at stage 4
        x_32d = self.gruUnit(x_32d, self.de_gru_4_row)
        # Finish de GRU at stage 4
        x_31d = F.relu(self.decoder_convtr_31(x_32d))
        x_30d = F.relu(self.decoder_convtr_30(x_31d))
        dim_3d = x_30d.size()
        ...
        x_22d = F.relu(self.decoder_convtr_22(x_2d))
        # de GRU at stage 3
        x_22d = self.gruUnit(x_22d, self.de_gru_3_row)
        # Finish de GRU at stage 3
        x_21d = F.relu(self.decoder_convtr_21(x_22d))
        x_20d = F.relu(self.decoder_convtr_20(x_21d))
        dim_2d = x_20d.size()
    ...
    def gruUnit(selfe, x, gru_model):
        stage2_shape = x.shape  # store the original feature shape
        x_gru = x.view(stage2_shape[0], -1,
                       stage2_shape[1])  # (batch_size, timesteps, channels"feature vectore for each Xt")
        x_gru, h_11 = gru_model(x_gru)  # get results after feeding it viewed feature map
        x_gru = x_gru.reshape(stage2_shape[0], -1, stage2_shape[2], stage2_shape[3])  # convert GRU result shape
        x_10 = torch.cat((x, x_gru), 1)
        return x_10
...
class SegNetGRU_Symmetric_columns(nn.Module):
    '''after each stage do the following
    1-save the orignal shape of the stage features
    2-create two views "row_view,column" of size (shape[0],-1,shape[1]) #batch,sequence(H*W),channels). row view needs to be transposed and used with reshape
    3-convert the GRU output to match the original shape
    ...
    def __init__(self, input_channels, num_classes):
        super(SegNetGRU_Symmetric_columns, self).__init__()
        ...
        # GRU stage 2
        self.gru_2_row = nn.GRU(input_size=128, hidden_size=64, num_layers=1, batch_first=True)
        self.gru_2_column = nn.GRU(input_size=128, hidden_size=64, num_layers=1, batch_first=True)
        ...
        # GRU stage 3
        self.gru_3_row = nn.GRU(input_size=256, hidden_size=128, num_layers=1, batch_first=True)
        self.gru_3_column = nn.GRU(input_size=256, hidden_size=128, num_layers=1, batch_first=True)
        ...
        # GRU stage 4
        self.gru_4_row = nn.GRU(input_size=512, hidden_size=256, num_layers=1, batch_first=True)
        self.gru_4_column = nn.GRU(input_size=512, hidden_size=256, num_layers=1, batch_first=True)
        self.encoder_conv_32 = nn.Sequential(*[
        ...
        # GRU stage 5
        self.gru_5_row = nn.GRU(input_size=512, hidden_size=256, num_layers=1, batch_first=True)
        self.gru_5_column = nn.GRU(input_size=512, hidden_size=256, num_layers=1, batch_first=True)
        self.encoder_conv_42 = nn.Sequential(*[
            nn.Conv2d(in_channels=512 + 512,
                      out_channels=512,
                      kernel_size=3,
        ...
        # de GRU stage 5
        self.de_gru_5_row = nn.GRU(input_size=512, hidden_size=256, num_layers=1, batch_first=True)
        self.de_gru_5_column = nn.GRU(input_size=512, hidden_size=256, num_layers=1, batch_first=True)
        self.decoder_convtr_41 = nn.Sequential(*[
        ...
        # de GRU stage 4
        self.de_gru_4_row = nn.GRU(input_size=512, hidden_size=256, num_layers=1, batch_first=True)
        self.de_gru_4_column = nn.GRU(input_size=512, hidden_size=256, num_layers=1, batch_first=True)
        self.decoder_convtr_31 = nn.Sequential(*[
            nn.ConvTranspose2d(in_channels=512+512,
                               out_channels=512,
                               kernel_size=3,
        ...
        # de GRU stage 3
        self.de_gru_3_row = nn.GRU(input_size=256, hidden_size=128, num_layers=1, batch_first=True)
        self.de_gru_3_column = nn.GRU(input_size=256, hidden_size=128, num_layers=1, batch_first=True)
        self.decoder_convtr_21 = nn.Sequential(*[
        ...
        # de GRU stage 2
        self.de_gru_2_row = nn.GRU(input_size=128, hidden_size=64, num_layers=1, batch_first=True)
        self.de_gru_2_column = nn.GRU(input_size=128, hidden_size=64, num_layers=1, batch_first=True)
        self.decoder_convtr_10 = nn.Sequential(*[
    ...
    def forward(self, input_img):
        ...
        x_10 = F.relu(self.encoder_conv_10(x_0))
        # GRU at stage 2
        x_10 = self.gruUnit(x_10, self.gru_2_row,self.gru_2_column)
        # Finish GRU at stage 2
        x_11 = F.relu(self.encoder_conv_11(x_10))
        x_1, indices_1 = F.max_pool2d(x_11, kernel_size=2, stride=2, return_indices=True)
        ...
        x_21 = F.relu(self.encoder_conv_21(x_20))
        # GRU at stage 3
        x_21 = self.gruUnit(x_21, self.gru_3_row,self.gru_3_column)
        # Finish GRU at stage 3
        x_22 = F.relu(self.encoder_conv_22(x_21))
        x_2, indices_2 = F.max_pool2d(x_22, kernel_size=2, stride=2, return_indices=True)
        ...
        x_31 = F.relu(self.encoder_conv_31(x_30))
        # GRU at stage 4
        x_31 = self.gruUnit(x_31, self.gru_4_row,self.gru_4_column)
        # Finish GRU at stage 4
        x_32 = F.relu(self.encoder_conv_32(x_31))
        x_3, indices_3 = F.max_pool2d(x_32, kernel_size=2, stride=2, return_indices=True)
        ...
        x_41 = F.relu(self.encoder_conv_41(x_40))
        # GRU at stage 5
        x_41 = self.gruUnit(x_41, self.gru_5_row,self.gru_5_column)
        # Finish GRU at stage 5
        x_42 = F.relu(self.encoder_conv_42(x_41))
        x_4, indices_4 = F.max_pool2d(x_42, kernel_size=2, stride=2, return_indices=True)
        ...
        x_42d = F.relu(self.decoder_convtr_42(x_4d))
        # de GRU at stage 5
        x_42d = self.gruUnit(x_42d, self.de_gru_5_row,self.de_gru_5_column)
        # Finish de GRU at stage 5
        x_41d = F.relu(self.decoder_convtr_41(x_42d))
        x_40d = F.relu(self.decoder_convtr_40(x_41d))
        dim_4d = x_40d.size()
        ...
        x_32d = F.relu(self.decoder_convtr_32(x_3d))
        # de GRU at stage 4
        x_32d = self.gruUnit(x_32d, self.de_gru_4_row,self.de_gru_4_column)
        # Finish de GRU at stage 4
        x_31d = F.relu(self.decoder_convtr_31(x_32d))
        x_30d = F.relu(self.decoder_convtr_30(x_31d))
        dim_3d = x_30d.size()
        ...
        x_22d = F.relu(self.decoder_convtr_22(x_2d))
        # de GRU at stage 3
        x_22d = self.gruUnit(x_22d, self.de_gru_3_row, self.de_gru_3_column)
        # Finish de GRU at stage 3
        x_21d = F.relu(self.decoder_convtr_21(x_22d))
        x_20d = F.relu(self.decoder_convtr_20(x_21d))
        dim_2d = x_20d.size()
        ...
        x_11d = F.relu(self.decoder_convtr_11(x_1d))
        # de GRU at stage 2
        x_11d = self.gruUnit(x_11d, self.de_gru_2_row, self.de_gru_2_column)
        # Finish de GRU at stage 2
        x_10d = F.relu(self.decoder_convtr_10(x_11d))
        dim_1d = x_10d.size()
...
class SegNetGRU_Symmetric_columns_shared_EncDec(nn.Module):
    '''
    The gru units are shared between encoding and decoding
    '''

    def __init__(self, input_channels, num_classes):
        super(SegNetGRU_Symmetric_columns_shared_EncDec, self).__init__()
    ...
    def forward(self, input_img):
        ...
        x_42d = F.relu(self.decoder_convtr_42(x_4d))
        # de GRU at stage 5
        x_42d = self.gruUnit(x_42d, self.gru_5_row,self.gru_5_column)
        # Finish de GRU at stage 5
        x_41d = F.relu(self.decoder_convtr_41(x_42d))
        x_40d = F.relu(self.decoder_convtr_40(x_41d))
        dim_4d = x_40d.size()
        ...
        x_32d = F.relu(self.decoder_convtr_32(x_3d))
        # de GRU at stage 4
        x_32d = self.gruUnit(x_32d, self.gru_4_row,self.gru_4_column)
        # Finish de GRU at stage 4
        x_31d = F.relu(self.decoder_convtr_31(x_32d))
        x_30d = F.relu(self.decoder_convtr_30(x_31d))
        dim_3d = x_30d.size()
        ...
        x_22d = F.relu(self.decoder_convtr_22(x_2d))
        # de GRU at stage 3
        x_22d = self.gruUnit(x_22d, self.gru_3_row, self.gru_3_column)
        # Finish de GRU at stage 3
        x_21d = F.relu(self.decoder_convtr_21(x_22d))
        x_20d = F.relu(self.decoder_convtr_20(x_21d))
        dim_2d = x_20d.size()
        ...
        x_11d = F.relu(self.decoder_convtr_11(x_1d))
        # de GRU at stage 2
        x_11d = self.gruUnit(x_11d, self.gru_2_row, self.gru_2_column)
        # Finish de GRU at stage 2
        x_10d = F.relu(self.decoder_convtr_10(x_11d))
        dim_1d = x_10d.size()
...
class SegNetGRU_Symmetric_columns_last2stages(nn.Module):
    '''
    The gru units are shared between encoding and decoding
    '''

    def __init__(self, input_channels, num_classes):
        super(SegNetGRU_Symmetric_columns_last2stages, self).__init__()
        ...
        self.encoder_conv_22 = nn.Sequential(*[
            nn.Conv2d(in_channels=256,  # add GRU feature map
                      out_channels=256,
                      kernel_size=3,
                      padding=1),
            nn.BatchNorm2d(256)
...
class SegNetGRU_Symmetric_columns_last2stages_Notshared_EncDec(nn.Module):
    '''
    The gru units are not shared between encoding and decoding
    GRU units are only applied to the last two stages
    '''

    def __init__(self, input_channels, num_classes):
        super(SegNetGRU_Symmetric_columns_last2stages_Notshared_EncDec, self).__init__()
    ...
    def forward(self, input_img):
        ...
        x_42d = F.relu(self.decoder_convtr_42(x_4d))
        # de GRU at stage 5
        x_42d = self.gruUnit(x_42d, self.de_gru_5_row,self.de_gru_5_row)
        # Finish de GRU at stage 5
        x_41d = F.relu(self.decoder_convtr_41(x_42d))
        x_40d = F.relu(self.decoder_convtr_40(x_41d))
        dim_4d = x_40d.size()
        ...
        x_32d = F.relu(self.decoder_convtr_32(x_3d))
        # de GRU at stage 4
        x_32d = self.gruUnit(x_32d, self.de_gru_4_row,self.de_gru_4_row)
        # Finish de GRU at stage 4
        x_31d = F.relu(self.decoder_convtr_31(x_32d))
        x_30d = F.relu(self.decoder_convtr_30(x_31d))
        dim_3d = x_30d.size()
...
class SegNetGRU_Symmetric_columns_last2stages_Notshared_EncDec_smallerH(nn.Module):
    '''
    The gru units are not shared between encoding and decoding
    GRU units are only applied to the last two stages
    '''
    ...
    def __init__(self, input_channels, num_classes):
        ...
        # GRU stage 4
        self.gru_4_row = nn.GRU(input_size=512, hidden_size=1, num_layers=1, batch_first=True)
        ...
        # GRU stage 5
        self.gru_5_row = nn.GRU(input_size=512, hidden_size=1, num_layers=1, batch_first=True)
        ...
        # de GRU stage 5
        self.de_gru_5_row = nn.GRU(input_size=512, hidden_size=1, num_layers=1, batch_first=True)

        self.decoder_convtr_41 = nn.Sequential(*[
            nn.ConvTranspose2d(in_channels=512+2,
                               out_channels=512,
                               kernel_size=3,
                               padding=1),
            nn.BatchNorm2d(512)
        ...
        # de GRU stage 4
        self.de_gru_4_row = nn.GRU(input_size=512, hidden_size=1, num_layers=1, batch_first=True)
    ...
    def gruUnit(self, x, gru_model_row,gru_model_column):
        ...
        #rows GRU
        x_gru_row = self.apply_GRU(x,gru_model_row)
        x_gru_row = F.softmax(x_gru_row,dim=2)
        #columns GRU
        x_transposed = x.transpose(2,3)
        x_gru_column = self.apply_GRU(x_transposed, gru_model_column)
        x_gru_column = F.softmax(x_gru_column,dim=3)
        x_gru_column = x_gru_column.transpose(2,3)
...
class SegNetGRU_5thStage_only_not_shared(nn.Module):
    '''
    The gru units are not shared between encoding and decoding
    GRU units are only applied to the fifth stage
    '''

    def __init__(self, input_channels, num_classes):
        super(SegNetGRU_5thStage_only_not_shared, self).__init__()
...
class SegNetGRU_4thStage_only_not_shared(nn.Module):
    '''
    The gru units are not shared between encoding and decoding
    GRU units are only applied to the last two stages
    '''
    ...
    def __init__(self, input_channels, num_classes):
        super(SegNetGRU_4thStage_only_not_shared, self).__init__()
...
class SegNetGRU_Symmetric_last2stages_FromEncToDec(nn.Module):
    '''
    The gru units are not shared between encoding and decoding
    GRU units are onlySegNetGRU_Symmetric_columns_UltimateShare applied to the last two stages
    '''

    def __init__(self, input_channels, num_classes):
        super(SegNetGRU_Symmetric_last2stages_FromEncToDec, self).__init__()
    ...
    def forward(self, input_img):
        ...
        x_31 = F.relu(self.encoder_conv_31(x_30))
        # GRU at stage 4
        x_31_GRU = self.gruUnit(x_31, self.gru_4_row,self.gru_4_row)
        # Finish GRU at stage 4
        x_32 = F.relu(self.encoder_conv_32(x_31))
        x_3, indices_3 = F.max_pool2d(x_32, kernel_size=2, stride=2, return_indices=True)
        ...
        x_41 = F.relu(self.encoder_conv_41(x_40))
        # GRU at stage 5
        x_41_GRU = self.gruUnit(x_41, self.gru_5_row,self.gru_5_row)
        # Finish GRU at stage 5
        x_42 = F.relu(self.encoder_conv_42(x_41))
        x_4, indices_4 = F.max_pool2d(x_42, kernel_size=2, stride=2, return_indices=True)
    ...
    def gruUnit(self, x, gru_model_row,gru_model_column):
        ...
        x_cated = torch.cat((x_gru_row,x_gru_column), 1)
        return x_cated
